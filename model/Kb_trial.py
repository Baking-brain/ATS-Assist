...
df = pd.read_csv("final_keyword_dataset.csv")  # Ensure it has 'Job Role' and 'Keywords' columns
...
model.load_state_dict(torch.load("bert_keywords.pth", map_location="cpu"))
model.eval()

# 🔹 Function to Predict Keywords
def predict_keywords(text, model, tokenizer, df, mlb, threshold=0.3):
